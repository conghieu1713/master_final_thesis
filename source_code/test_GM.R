# ============================================================================== #
# ------------                                                       ----------- #
# ------------    ƯỚC LƯỢNG MÔ HÌNH UNIVARIATE GARCH-MIDAS           ----------- #
# ------------                                                       ----------- #
# ============================================================================== #
...
# ------------------------------------------------------------------------------ #
  ...
  # ============================================================================ #
    ...
    # Đặt là TRUE nếu bạn muốn loại bỏ tự tương quan trong chuỗi lợi suất bằng ARMA(1,1)
    do_arma_spec = TRUE
...
# ------------------------------------------------------------------------------ #
  prepare_ugm_data <- function(asset, rate_data, K_1, midas1_data, midas1_col, midas1_freq, do_arma = FALSE, garchx_data = NULL, garchx_col = NULL, K_2 = NULL, midas2_data = NULL, midas2_col = NULL, midas2_freq = NULL) {
    ...
    if (do_arma) {
      ...
      # Sử dụng forecast::Arima với bậc (1,0,1) cố định
      ...
      # Sử dụng forecast::auto.arima để tự động chọn mô hình ARMA tốt nhất
      # stationary = TRUE để đảm bảo chuỗi kết quả là dừng
      # allowdrift = FALSE để không bao gồm hằng số trôi dạt
      arma_fit <- forecast::auto.arima(daily_ret_base, stationary = TRUE, trace = FALSE, allowdrift = FALSE) 
      
      # Lấy bậc của mô hình được chọn để thông báo
      selected_order <- arma_fit$arma[c(1, 6, 2)] # p, d, q (d sẽ là 0 do stationary=TRUE)
...
# ------------------------------------------------------------------------------ #
    # Khởi tạo danh sách để lưu trữ phần dư của tất cả tài sản
    all_residuals <- list()

    # Bắt đầu vòng lặp qua từng tài sản đã định nghĩa trong `asset_midas_map`
    for (asset_name in names(asset_midas_map)) {
      midas1_col_spec <- asset_midas_map[[asset_name]]
      ...
      # 4. Trích xuất, kiểm định và lưu phần dư chuẩn hóa
      if (!is.null(ugm_fit)) {
        ...
        cat(sprintf("\n>> Bỏ qua lưu phần dư cho '%s' do lỗi ước lượng.\n", asset_name))
      }
    }
