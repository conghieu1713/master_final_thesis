# ============================================================================== #
# ------------                                                       ----------- #
# ------------   ƯỚC LƯỢNG DCC-MIDAS-X (TRONG MẪU & NGOÀI MẪU)       ----------- #
# ------------                                                       ----------- #
# ============================================================================== #
...
library(dplyr)
library(tidyr)
library(numDeriv) # Thêm thư viện này để tính Hessian

# ------------------------------------------------------------------------------ #
# -- 2. THIẾT LẬP CÁC THAM SỐ CHO MÔ HÌNH -------------------------------------- #
# ------------------------------------------------------------------------------ #
# NGÀY CẮT DỮ LIỆU
# Ngày này sẽ chia dữ liệu thành hai tập: "In-Sample" (trước và bằng ngày cắt)
# và "Out-of-Sample" (sau ngày cắt).
# Đặt là NULL để chạy trên toàn bộ dữ liệu (chỉ có 1 tập "Full-Sample").
...
# TẠO CÁC KỊCH BẢN THỜI GIAN ĐỂ CHẠY
time_periods <- list()
if (!is.null(cutoff_date)) {
  cat(sprintf("Điểm cắt thời gian được đặt là: %s\n", cutoff_date))
  cat("Chương trình sẽ chạy cho 2 giai đoạn: In-Sample và Out-of-Sample.\n")
  time_periods <- list(
    list(name = "In-Sample", start_date = NULL, end_date = as.Date(cutoff_date)),
    list(name = "Out-of-Sample", start_date = as.Date(cutoff_date) + 1, end_date = NULL)
  )
} else {
  cat("Không có điểm cắt thời gian. Chương trình sẽ chạy trên toàn bộ mẫu.\n")
  time_periods <- list(
    list(name = "Full-Sample", start_date = NULL, end_date = NULL)
  )
}
...
# -- 3.1. HÀM TẢI VÀ KHỚP DỮ LIỆU (VỚI KHOẢNG THỜI GIAN TÙY CHỈNH) ------------- #
load_and_align_data <- function(z_file_path, asset1, asset2, x_file_path, x_sheet, x_var, start_date = NULL, end_date = NULL) {
  ...
  # Áp dụng khoảng thời gian nếu được chỉ định
  if (!is.null(start_date) || !is.null(end_date)) {
    cat(sprintf(">> Áp dụng khoảng thời gian: từ %s đến %s.\n", 
                ifelse(is.null(start_date), "đầu tiên", as.character(start_date)), 
                ifelse(is.null(end_date), "cuối cùng", as.character(end_date))))
    z_xts <- window(z_xts, start = start_date, end = end_date)
    x_xts_monthly <- window(x_xts_monthly, start = start_date, end = end_date)
  }
...
report_dcc_x_results <- function(optim_results, z_matrix, x_monthly, month_map, K_lags, asset1, asset2, x_var, period_name) {
  ...
  cat("========================================================================\n")
  cat(sprintf(" Giai đoạn phân tích: %s\n", period_name))
...
# ------------------------------------------------------------------------------ #
# -- 4. THỰC THI MÔ HÌNH CHO CÁC GIAI ĐOẠN VÀ KỊCH BẢN ------------------------- #
# ------------------------------------------------------------------------------ #

# Vòng lặp qua từng giai đoạn thời gian (In-Sample, Out-of-Sample, v.v.)
for (period_config in time_periods) {
  
  cat(sprintf("\n\n##########################################################################"))
  cat(sprintf("\n#####   BẮT ĐẦU CHẠY KỊCH BẢN CHO GIAI ĐOẠN: %s   #####", period_config$name))
  cat(sprintf("\n##########################################################################\n"))
  ...
  for (x_config in exogenous_vars_config) {
    ...
    for (asset_config in asset_pairs_config) {
      ...
      # 1. Tải và chuẩn bị dữ liệu cho giai đoạn cụ thể
      data_list <- tryCatch({
        load_and_align_data(
      ...
      if (is.null(data_list) || nrow(data_list$z_matrix) < (K_lags * 21 + 50)) { # Thêm kiểm tra số quan sát
        cat(">> Bỏ qua do không đủ dữ liệu cho giai đoạn này.\n")
        next 
      }
      ...
      report_dcc_x_results(
        ...
        asset2 = asset2,
        x_var = x_var,
        period_name = period_config$name # Truyền tên giai đoạn
      )
      
      cat(sprintf("\n HOÀN TẤT XỬ LÝ: Cặp [%s-%s] với biến X [%s] cho giai đoạn %s\n", 
                  asset1, asset2, x_var, period_config$name))
    }
  }
}

cat("\n\n##########################################################################")
cat("\n#####                    TẤT CẢ KỊCH BẢN ĐÃ HOÀN TẤT                  #####")
cat("\n##########################################################################\n")
