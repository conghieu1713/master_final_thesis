# ============================================================================== #
# ------------                                                       ----------- #
# ------------      ƯỚC LƯỢNG MÔ HÌNH DCC-MIDAS-X (VỚI TIME SPLIT)   ----------- #
# ------------                                                       ----------- #
# ============================================================================== #
...
# ------------------------------------------------------------------------------ #
# NGÀY CẮT DỮ LIỆU (tùy chọn)
# Đặt là một chuỗi ngày tháng "YYYY-MM-DD" để cắt dữ liệu đến ngày đó.
# Đặt là NULL để sử dụng toàn bộ dữ liệu.
cutoff_date <- "2019-12-31" # Ví dụ: chỉ phân tích dữ liệu đến cuối năm 2019
...
# -- 3.1. HÀM TẢI VÀ KHỚP DỮ LIỆU (VỚI TÙY CHỌN CẮT THỜI GIAN) ----------------- #
load_and_align_data <- function(z_file_path, asset1, asset2, x_file_path, x_sheet, x_var, cutoff_date = NULL) {
  ...
  # Áp dụng điểm cắt thời gian nếu được chỉ định
  if (!is.null(cutoff_date)) {
    cat(sprintf(">> Áp dụng điểm cắt thời gian: giữ lại dữ liệu đến ngày %s.\n", cutoff_date))
    z_xts <- window(z_xts, end = as.Date(cutoff_date))
    x_xts_monthly <- window(x_xts_monthly, end = as.Date(cutoff_date))
  }
...
report_dcc_x_results <- function(optim_results, z_matrix, x_monthly, month_map, K_lags, asset1, asset2, x_var) {
  ...
  if (!is.null(cutoff_date)) {
    cat(sprintf(" Dữ liệu được cắt đến ngày: %s\n", cutoff_date))
  }
  cat("------------------------------------------------------------------------\n")
  ...
  for (asset_config in asset_pairs_config) {
    ...
    data_list <- tryCatch({
      load_and_align_data(
        ...
        x_var = x_var,
        cutoff_date = cutoff_date # Truyền tham số điểm cắt vào hàm
      )
    }, error = function(e) {
